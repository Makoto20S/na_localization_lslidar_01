...
//bool   add_vision_flag = false;
double last_timestamp_lidar = 0, last_timestamp_imu = -1.0,last_timestamp_leg= -1.0,last_timestamp_vi= -1.0;
...
nav_msgs::Odometry odom_rpy_lla;
...
string leg_topic;
bool useleg;
double max_z = 0,min_z = 0;//记录构图过程中最大最小高度值，用于重定位给出高度

ros::Publisher pubSavedCloud;  // 用于发布保存的点云地图
ros::Publisher pubSavedPose;   // 用于发布保存的位姿信息
...
void standard_pcl_cbk(const sensor_msgs::PointCloud2::ConstPtr &msg) 
{
    ...
    if(!first_lidar)
        first_lidar = true;
    add_lidar_flag = true;  //通知其他线程，有新数据进入
    scan_count ++;   //计数器
    double preprocess_start_time = omp_get_wtime();
    if (msg->header.stamp.toSec() < last_timestamp_lidar)   //检测当前时间辍是否小于是上一次
    {
        ROS_ERROR("lidar loop back, clear buffer");
        lidar_buffer.clear();     //小于时出错，清空缓冲区
    }
    //点云预处理
    PointCloudXYZI::Ptr  ptr(new PointCloudXYZI());  //创建点云空指针
    p_pre->process(msg, ptr);   //将msg中的点云信息转换为PCL数据储存在ptr中

	//数据异常
    if (ptr->points.back().curvature / 1000.0 > 1.0)  //检查点云最后一个点的曲率是否异常，是否超过阈值（1.0），若超则异常
    {
        mtx_buffer.unlock();   //释放线程锁
        sig_buffer.notify_all(); //唤醒当前正在等待该条件的所有线程
        return;
    }
/*-------------------------------------------------------------------------------*/
//     //************将点云由左后上转到前左上*******************/
...
//         point.x = x_tmp;
//         point.y = y_tmp;
//         if(fabs(point.x)>10000)
//         cout<<"point_in : "<<point.x<<" , "<<point.y<<" , "<<point.z<<endl;
//     }
//        //旋转
//     for(auto& point : ptr->points)
//     {
...
/*---------------------------------------------------------------------------------*/
    ...
    mtx_buffer.unlock();  //释放
    sig_buffer.notify_all(); //通知
}
...
void imu_cbk(const sensor_msgs::Imu::ConstPtr &msg_in) 
{
    ...
    // //cout<<"IMU数据: "<<msg->linear_acceleration.x<<", "<<msg->linear_acceleration.y<<","<<msg->linear_acceleration.z<<endl;
    // /*-------------------------------------------------------------------------------------------------------*/
...
void vision_cbk(const nav_msgs::Odometry::ConstPtr &msg_in)
{
...
void publish_odometry(const ros::Publisher & pubOdomAftMapped,const ros::Publisher & pubOdom_rpy_lla)
{
    ...
    Eigen::Vector3d llapose;
    //cout<<"输入经维高："<<longitude_<<","<<latitude_<<","<<msg_in->altitude<<endl;
    llapose = gnss2enu->ENUToLLA(odomAftMapped.pose.pose.position.x,odomAftMapped.pose.pose.position.y,odomAftMapped.pose.pose.position.z);
    //std::cout<<"定位结果（局部东北天坐标）："<<" x = "<<odomAftMapped.pose.pose.position.x<<" y = "<<odomAftMapped.pose.pose.position.y<<" z =  "<<odomAftMapped.pose.pose.position.z<<std::endl;
    
    //printf("定位结果（经纬度坐标）： 经度 = %3.7lf; 纬度 =  %3.7lf;  高度 =  %3.3lf; \n", llapose[0]/M_PI*180, llapose[1]/M_PI*180, llapose[2]);

    geometry_msgs::Quaternion geoQuat_test =  odomAftMapped.pose.pose.orientation;
    double roll_test, pitch_test, yaw_test;
  	tf::Matrix3x3(tf::Quaternion(geoQuat_test.x, geoQuat_test.y, geoQuat_test.z, geoQuat_test.w)).getRPY(roll_test, pitch_test, yaw_test);
    //std::cout<<"横滚角 =  "<<roll_test*180/M_PI<<" 俯仰角 =  "<<pitch_test*180/M_PI<<" 航向角  =  "<<yaw_test*180/M_PI<<std::endl;
    double enu_yaw;
    enu_yaw = 90.0 - yaw_test*180/M_PI;
    if(enu_yaw < 0)
    {
        enu_yaw += 360.0;
    }
    //std::cout<<"***东北天航向角(北偏东): "<<enu_yaw<<endl;
    pubOdomAftMapped.publish(odomAftMapped);
    if(odomAftMapped.pose.pose.position.z > max_z)
    max_z = odomAftMapped.pose.pose.position.z;
    if(odomAftMapped.pose.pose.position.z < min_z)
    min_z = odomAftMapped.pose.pose.position.z;
    //发布东北天位置、姿态角、经纬高位置
    odom_rpy_lla.header.frame_id = "camera_init";
    odom_rpy_lla.child_frame_id = "body";
    odom_rpy_lla.header.stamp = ros::Time().fromSec(lidar_end_time);
    //东北天位置
    odom_rpy_lla.pose.pose.position.x = odomAftMapped.pose.pose.position.x;
    odom_rpy_lla.pose.pose.position.y = odomAftMapped.pose.pose.position.y;
    odom_rpy_lla.pose.pose.position.z = odomAftMapped.pose.pose.position.z;
    //姿态角
    odom_rpy_lla.pose.pose.orientation.x = roll_test*180/M_PI;
    odom_rpy_lla.pose.pose.orientation.y = pitch_test*180/M_PI;
    odom_rpy_lla.pose.pose.orientation.z = yaw_test*180/M_PI;
    //经纬高位置
    odom_rpy_lla.twist.twist.linear.x = llapose[0]/M_PI*180;
    odom_rpy_lla.twist.twist.linear.y = llapose[1]/M_PI*180;
    odom_rpy_lla.twist.twist.linear.z = llapose[2]/M_PI*180;
    pubOdom_rpy_lla.publish(odom_rpy_lla);
...
void publish_path(const ros::Publisher pubPath)
{
    ...
    if (jjj % 1 == 0) 
    {
        ...
        slam_time.push_back(lidar_end_time);
...
bool saveMapService(na_mapping::save_mapRequest& req, na_mapping::save_mapResponse& res)
{
    ...
    pcl::toROSMsg(*MapKeyFramesDS, cloudMsg);
    ...
    if(1)
    {
        ...
        fprintf(fp,"%2.10lf %3.10lf %2.5lf %2.2f %2.2f",lat0*180/M_PI,lon0*180/M_PI,alt0,max_z,min_z);
        fclose(fp); 
        ...
        std::stringstream ss;
        ss << std::fixed << std::setprecision(10) << lat0*180/M_PI << " "
           << std::fixed << std::setprecision(10) << lon0*180/M_PI << " "
           << std::fixed << std::setprecision(5) << alt0 << " "
           << std::fixed << std::setprecision(2) << max_z << " "
           << std::fixed << std::setprecision(2) << min_z;
        poseMsg.data = ss.str();
...
void gnss_cbk_sensor_msgs_2enu(const sensor_msgs::NavSatFix::ConstPtr& msg_in)
{
    ...
    //cout<<"msg_in->latitude = "<<msg_in->latitude<<endl;
    latitude_ = msg_in->latitude;
    longitude_= msg_in->longitude;
    if(0) 
    {
            double tmp_lat, int_lat;
    tmp_lat = msg_in->latitude;
    int_lat = (int)tmp_lat;
    tmp_lat = (tmp_lat - (double)int_lat)*10.0/6.0;
    latitude_ = (double)int_lat + tmp_lat;

    double tmp_lon, int_lon;
    tmp_lon = msg_in->longitude;
    int_lon = (int)tmp_lon;
    tmp_lon = (tmp_lon - (double)int_lon)*10.0/6.0;
    longitude_ = (double)int_lon + tmp_lon;
    }
    //cout<<"latitude_ = "<<latitude_<<endl;
    Eigen::Vector3d ENUpose;
    //printf("真值经纬度坐标： 经度 = %3.7lf; 纬度 =  %3.7lf;  高度 =  %3.3lf; \n", longitude_, latitude_, msg_in->altitude);
    //初始化局部东北天坐标系原点
    if(!rtk_p0_init)
    {
        //lat0 = latitude_;
        //lon0 = longitude_;
        lat0 = latitude_*M_PI/180;
        lon0 = longitude_*M_PI/180;
        alt0 = msg_in->altitude;    
        gnss2enu->SetOriginLLA(lon0,lat0,alt0);
        cout<<endl;
        cout<<endl;
    ...
    //shared_ptr<LLAENU> p_llaenu = std::make_shared<LLAENU>(lat0, lon0, alt0);
    //ENUpose = p_llaenu->LLAToENU(latitude_,longitude_,msg_in->altitude);
    ...
    Eigen::Vector3d enupose;
    //cout<<"输入经维高："<<longitude_<<","<<latitude_<<","<<msg_in->altitude<<endl;
    enupose = gnss2enu->LLAToENU(longitude_*M_PI/180,latitude_*M_PI/180,msg_in->altitude);
    //cout<<"enupose:::"<<enupose[0]<<","<<enupose[1]<<","<<enupose[2]<<endl;
    //cout<<"原点经维高："<<gnss2enu->longitude_<<","<<gnss2enu->latitude_<<","<<gnss2enu->altitude_<<endl;
    //GeographicLib::UTMUPS::Forward(latitude_, longitude_, zone, northp, utm_x, utm_y);
    //utm_z = msg_in->altitude;
    //utm_x -= utm_x0;
    //utm_y -= utm_y0;
    //utm_z -= utm_z0;
    ...
    gnss_data_enu.pose.pose.position.x =  enupose[0] ;  //东
    gnss_data_enu.pose.pose.position.y =  enupose[1] ;  //北
    gnss_data_enu.pose.pose.position.z =  enupose[2] ;  //天
...
void gnss_cbk_sensor_msgs(const sensor_msgs::NavSatFix::ConstPtr& msg_in)
{
    if (usertk==false)
    return;
...
int main(int argc, char** argv)
{
    ...
    nh.param<std::string>("savemappath", savemappath, "/home/firefly/na_mapping_lslidar/src/na_mapping/PCD/cloud_map.pcd"); // 保存地图点云的路径
    nh.param<std::string>("saveposepath", saveposepath, "/home/firefly/na_mapping_lslidar/src/na_mapping/PCD/pose.txt");    // 保存地图原点的位置（只有用gps的时候会保存）
    ...
    ros::Subscriber sub_imu = nh.subscribe(imu_topic, 200000, imu_cbk,ros::TransportHints().unreliable());  //采用UDP，延迟低，丢包率高  ros::TransportHints().unreliable()
    ...
    ros::Subscriber sub_vision_odom = nh.subscribe("/vins_estimator/odometry", 200000, vision_cbk);
    ...
    ros::Publisher pubOdom_rpy_lla = nh.advertise<nav_msgs::Odometry> ("/Odom_rqy_lla", 100000);
    ...
    ros::Publisher pubTestCloud = nh.advertise<sensor_msgs::PointCloud2>("/test_cloud", 100000);
    ...
    pubGPS_flag =nh.advertise<std_msgs::Bool>("/GPS_add_flag",1);
    ...
    pubSavedCloud = nh.advertise<sensor_msgs::PointCloud2>("/saved_cloud_map", 1);
    pubSavedPose = nh.advertise<std_msgs::String>("/saved_pose_info", 1);
    ...
    while (ros::ok())
    {
        ...
        {   
            ...
            p_imu1->Process(Measures, kf, feats_undistort);   //初始化IMU参数，更新滤波器状态和协方差矩阵
            ...
            pos_lid = state_point.pos + state_point.rot.matrix() * state_point.offset_T_L_I;  //雷达在world系下的位置
            ...
            pcl::toROSMsg(*feats_down_body, laserCloudtest);   //将PCL的点云数据转化为ROS中的数据
            laserCloudtest.header.stamp = ros::Time().fromSec(lidar_end_time);
            laserCloudtest.header.frame_id = "body";
            pubTestCloud.publish(laserCloudtest);
            //cout<<"feats_down_size : "<<feats_down_size<<endl;
            //if (feats_down_size > 10000)
            //{
            //    for(auto &point : feats_undistort->points)
            //    {
            ...
            if(ikdtree.Root_Node == nullptr)
            {
                ...
                feats_down_world->resize(feats_down_size); //调整feats_down_world的大小，使与feats_down_body一样
                for(int i = 0; i < feats_down_size; i++) //将feats_down_body的点转到feats_down_world
                {
            ...
            if(1) 
            {
                ...
                ikdtree.flatten(ikdtree.Root_Node, ikdtree.PCL_Storage, NOT_RECORD);  //将ikdtree中的点转到PCL_Storage中
                featsFromMap->clear(); //清空
            ...
            state_point = kf.get_x();
            if(fabs(state_point.pos[0])>100)
            cout<<"state_point :"<<state_point.pos<<endl;
            ...
            cout<<"后端融合有"<<sensor_num<<"个传感器,分别是：";
            if(add_imu_flag)
            {
                cout<<"imu 、 ";
            }
            ...
            cout<<endl;
            add_vision_flag = false;
            add_imu_flag = false;
            add_GPS_flag = false;
            add_lidar_flag = false;
            ...
            /******* Publish odometry *******/
            publish_odometry(pubOdomAftMapped,pubOdom_rpy_lla);
    ...
    if(0)
    {
        string file_name1 = "/home/firefly/na_mapping_Lslidar/src/na_mapping/Path/lidar_path.txt";//关键帧
        string file_name2 = "/home/firefly/na_mapping_Lslidar/src/na_mapping/Path/rtk_path.txt";
        string file_name3 = "/home/firefly/na_mapping_Lslidar/src/na_mapping/Path/slam_path.txt";
        ofstream fout;
    ...
    cout<<"slamPath size:"<<path.poses.size()<<endl;
    cout<<"slam_time size:"<<slam_time.size()<<endl;
    ...
    {   
            ...
            pos_imu << path.poses[i].pose.position.x,
                    path.poses[i].pose.position.y,
                    path.poses[i].pose.position.z;

            q_imu.w() = path.poses[i].pose.orientation.w;
            q_imu.x() = path.poses[i].pose.orientation.x;
            q_imu.y() = path.poses[i].pose.orientation.y;
            q_imu.z() = path.poses[i].pose.orientation.z;

            pos_rtk = pos_imu + q_imu.normalized().toRotationMatrix() * rtk_T_wrt_Lidar;

            fout << fixed << setprecision(4) << path.poses[i].pose.position.x << " "
                << fixed << setprecision(4) << path.poses[i].pose.position.y << " "
                << fixed << setprecision(4) << path.poses[i].pose.position.z << endl;
