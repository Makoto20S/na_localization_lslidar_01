...
using SCPointType = utils::PointI; // using xyz only. but a user can exchange the original bin encoding function (i.e., max hegiht) to max intensity (for detail, refer 20 ICRA Intensity Scan Context)
...
public: 
    ...
    void deleteLastScancontext();
    bool SavetoTxt(const string& fileDirectory);
    bool ReadformTxt(const string& fileDirectory, int& nums);
    void deletethisKeyframe(int thisKeyframeID);

    std::pair<int, float> detectLoopClosureID( void ); // int: nearest node index, float: relative yaw  

    void calculateAngle(pcl::PointCloud<SCPointType> & _scan_down, int idx, float& yaw_diff_rad);

public:
    // hyper parameters ()
    const double LIDAR_HEIGHT = 0; // lidar height : add this for simply directly using lidar scan in the lidar local coord (not robot base coord) / if you use robot-coord-transformed lidar scans, just set this as 0.

    const int    PC_NUM_RING = 25; // 20 in the original paper (IROS 18)
    const int    PC_NUM_SECTOR = 90; // 60 in the original paper (IROS 18)
    const double PC_MAX_RADIUS = 100.0; // 80 meter max in the original paper (IROS 18)
    const double PC_UNIT_SECTORANGLE = 360.0 / double(PC_NUM_SECTOR);
    const double PC_UNIT_RINGGAP = PC_MAX_RADIUS / double(PC_NUM_RING);

    // tree
    const int    NUM_EXCLUDE_RECENT = 2; // simply just keyframe gap, but node position distance-based exclusion is ok. 
    const int    NUM_CANDIDATES_FROM_TREE = 1000; // 10 is enough. (refer the IROS 18 paper)

    // loop thres
    const double SEARCH_RATIO = 1; // for fast comparison, no Brute-force, but search 10 % is okay. // not was in the original conf paper, but improved ver.
