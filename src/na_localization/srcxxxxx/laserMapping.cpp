...
void gnss_cbk_sensor_msgs_2enu(const sensor_msgs::NavSatFix::ConstPtr &msg_in)
{
    ...
    if(msg_in->status.status < status_threshold)
        return;
    //  ROS_INFO("GNSS DATA IN ");
    double timestamp = msg_in->header.stamp.toSec() + rtk_time_grift;
    ...
    rtk_status = msg_in->status.status;
...
void ManualPos_cbk(const geometry_msgs::PoseStamped::ConstPtr &msg_in)
{
    ...
    if (flag_manualpos==false && need_relocal == true)
    {
...
void WebPos_cbk(const geometry_msgs::PoseStamped::ConstPtr &msg_in)
{
    // if (Reposition_type!=2)
    //     return; 
    ...
    if (flag_manualpos==false && need_relocal == true)
    {
       ...
       cout<<"Receive web External Position Successful!" << manualpos[0] << "," << manualpos[1] << "," << manualpos[2] <<endl;
       cout<<manualpos[0]<<" "<<manualpos[1]<<" "<<manualpos[2]<<endl;
    } 
...
void manualpos_reposition2()
{
    ...
    if(!read_keytxt)
    {
        string keyposepath = "/home/firefly/na_localization_lslidar/src/na_localization/PCD/lidar_path.txt";
...
void rtk_reloc()
{
    ...
    double search_radius_modi = rtk_status < 4 ? search_radius_rtk : search_radius_rtk / 2;
    cout << "位姿可能半径： " << search_radius_modi << endl;
    bool reloSuccess_ = false;
    utils::Pose init_pose(pos_repos.x, pos_repos.y, 0.0, 0, 0, 0);

    reloSuccess_ = reloc_plugin_ptr_->localRelocByBfs(init_pose, search_radius_modi);
    ...
    if (reloSuccess_)
    {
        ...
        cout << "RTK局部重定位成功, publish Relocal_flag_msg" << endl;
    }
    else
    {
        // need_relocal = false;
        ...
        Relocal_flag_msg.data = false;
        pubRelocal_flag.publish(Relocal_flag_msg);
        cout << "RTK局部重定位失败, publish Relocal_flag_msg" << endl;
    }
}
...
void ReLocalization()
{
    ...
    if (!need_relocal)
    {
        ...
        {
            std::lock_guard<std::mutex> lock(MM.mutex_);
            ...
            // *cloud_copy_1 = *(MM.pointcloud_output);
            // kdtree1.setInputCloud(cloud_copy_1);

            // cout << "start reset ikdtree 3" << endl;

            // pcl::PointCloud<PointType>::Ptr cloud_copy_2;
            // cloud_copy_2.reset(new pcl::PointCloud<PointType>());
            // *cloud_copy_2 = *(MM.pointcloud_output);
            // kdtree2.setInputCloud(MM.cloud_copy_2);
...
void sensor_vaild_judge(Eigen::Vector3d &z_leg, Eigen::Vector3d &z_rtk, Eigen::Matrix3d& z_sigma_rtk)
{
    // 判断leg的可用性
    ...
    {
        // 删除当前帧0.1s之前的rtk
        if (gnss_buffer.front().header.stamp.toSec() < (Measures.package_end_time - 0.1))
        {
            gnss_buffer.pop_front();
            continue;
        }
        // 超过当前帧0.1s之后，退出
        else if (gnss_buffer.front().header.stamp.toSec() > (Measures.package_end_time + 0.1))
        {
            break;
        }
        else
        {
            ...
            z_sigma_rtk(0, 0) = thisGPS.pose.covariance[0];
            z_sigma_rtk(1, 1) = thisGPS.pose.covariance[7];
            z_sigma_rtk(2, 2) = thisGPS.pose.covariance[14];
...
void lasermap_fov_segment()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (dist_to_map_edge[i][0] <= 80 || dist_to_map_edge[i][1] <= 80)
            need_move = true;
    }
    if (!need_move)
...
bool isKeyFrame(const Eigen::Vector3d& current_pos, const Eigen::Vector3d& last_pos, const Eigen::Matrix3d& current_rot, const Eigen::Matrix3d& last_rot, double keyframe_threshold_pos, double keyframe_threshold_rot)
{
   ...
   // Eigen::AngleAxisd rot_change(rot_diff);
    ...
    // Eigen::Vector3d euler_angles = rot_diff.eulerAngles(2, 1, 0);
    // double roll = euler_angles(0) * 180.0 / M_PI;
...
int main(int argc, char **argv)
{
    ...
    // bfsreloc
    nh.param<double>("Search_Radius", search_radius, 5.0); // 查找半径
    nh.param<double>("Search_Radius_rtk", search_radius_rtk, 15.0); // 查找半径

    cout << "Search_Radius_rtk:" << search_radius_rtk << "," << "Search_Radius:" << search_radius << endl;
    // rtk
    nh.param<bool>("usertk", usertk, false);                           // 是否使用rtk
    
    nh.param<string>("common/gnss_topic", gnss_topic, "/rtk_pos_raw"); // gps的topic名称
    nh.param<int>("status_threshold", status_threshold, 4);

    nh.param<string>("common/gnss_heading_topic", gnss_heading_topic, "/rtk_heading_raw"); // gps_heading的topic名称

    nh.param<std::string>("loadmappath", loadmappath, "/home/firefly/na_mapping_Lslidar/src/na_mapping/PCD/cloud_map.pcd");             // 加载地图的路径
    nh.param<std::string>("loadposepath", loadposepath, "/home/firefly/na_mapping_Lslidar/src/na_mapping/PCD/pose.txt");                // 加载初始位置的路径（只有用rtk才需要）
    nh.param<std::string>("params_filename", params_filename, "/home/firefly/na_localization_general/src/na_localization/PCD/param.json"); // 加载重定位
    ...
    nh.param<bool>("chengxin_change", chengxin_change, false); 
    ...
    ros::Subscriber sub_pcl = p_pre->lidar_type == AVIA ? nh.subscribe(lid_topic, 1, livox_pcl_cbk) : nh.subscribe(lid_topic, 1, standard_pcl_cbk);
    ...
    ros::Subscriber sub_ExterPos=nh.subscribe("/move_base_simple/goal",1,ManualPos_cbk); //手动重定位话题
    ros::Subscriber sub_ManualPos=nh.subscribe("/web_loc_goal_pose",1,WebPos_cbk); //web重定位话题///move_base_simple/goal
    ...
    // string params_filename = string("/home/firefly/na_localization_lslidar/src/na_localization/PCD/param.json");
    ...
    while (ros::ok())
    {
        ...
        //    cout << "预测：" << state_point.pos[0] << "," << state_point.pos[1] << "," << state_point.pos[2] << endl;
            ...
            // 判断传感器的可用性
            sensor_vaild_judge(z_leg, z_rtk, Sigma_rtk);
           ...
           // rtk_vaild = false;
            ...
            if (status_tree1 == 0 && last_ikdtree == 1)
            {
                ...
                bool kf_converge = kf.update_iterated_dyn_share_modified(LASER_POINT_COV, feats_down_body, kdtree1, Nearest_Points, NUM_MAX_ITERATIONS, extrinsic_est_en, effct_feat_num_, vaild_points_,
                ...
                if(kf_converge == false) //if(dyn_share.valid == false)
                {
                    std_msgs::Bool Local_flag_msg;
                    Local_flag_msg.data = kf_converge;
					
					pubLocal_flag.publish(Local_flag_msg);
                    ROS_ERROR("定位丢失!! 发布定位丢失消息");
                    if (usertk)
                    {
                        need_relocal = true;
                        // flag_manualpos = true;
            ...
            else if (status_tree2 == 0 && last_ikdtree == 2)
            {
                ...
                double time_kf1 = omp_get_wtime();
                bool kf_converge = kf.update_iterated_dyn_share_modified(LASER_POINT_COV, feats_down_body, kdtree2, Nearest_Points, NUM_MAX_ITERATIONS, extrinsic_est_en, effct_feat_num_, vaild_points_,
                                                      Sigma_leg, z_leg, useleg, Measures.leg_vaild, Measures.lidar_vaild, need_relocal, Sigma_rtk, z_rtk, rtk_vaild, cur_atti, kf_heading, rtk_heading_vaild, pubLocal_flag, pubCloudeffct, lidar_end_time);
                // cout<<"effct_feat_num_en2 = "<<effct_feat_num_en<<endl;
                if(kf_converge == false)
                {
                    std_msgs::Bool Local_flag_msg;
                    Local_flag_msg.data = kf_converge;
       ...
       //关键帧提取
            if (isKeyFrame(state_point.pos, state_point_lastframe.pos, state_point.rot.matrix(), state_point_lastframe.rot.matrix(), keyframe_threshold_pos, keyframe_threshold_rot)||keyframe_count==0)
            {
                ...
                // Eigen::Vector3d euler_angles = state_point.rot.matrix().eulerAngles(2, 1, 0);
                // keyframe_pose.roll = euler_angles(0) * 180.0 / M_PI;
                // keyframe_pose.pitch = euler_angles(1) * 180.0 / M_PI;
                // keyframe_pose.yaw = euler_angles(2) * 180.0 / M_PI;
                // keyframe_pose.yaw = fmod(keyframe_pose.yaw + 360.0, 360.0);
                Eigen::Quaterniond q_internal = state_point.rot.unit_quaternion();
                tf::Quaternion q1(q_internal.x(), q_internal.y(), q_internal.z(), q_internal.w());//四元数初始化参数顺序为x,y,z,w
                double roll, pitch, yaw;
                tf::Matrix3x3(q1).getRPY(roll, pitch, yaw);
                keyframe_pose.roll = roll* 180.0 / M_PI;//度
                keyframe_pose.pitch = pitch* 180.0 / M_PI;
                keyframe_pose.yaw = yaw* 180.0 / M_PI;
