    ...
    for (uint i = 1; i < plsize; i++)
    {
      if ((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
      {
        ...
        if (valid_num % point_filter_num == 0)
        {
          ...
          {
            if(abs(pl_full[i].x)  > 0.65 || abs(pl_full[i].y) > 0.3) //去除狗周围坐标
...
void Preprocess::velodyne_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  else
  {
    for (int i = 0; i < plsize; i++)
    {
      ...
      if (i % point_filter_num == 0)
      {
        if (added_pt.x * added_pt.x + added_pt.y * added_pt.y + added_pt.z * added_pt.z > (blind * blind) && added_pt.x * added_pt.x + added_pt.y * added_pt.y + added_pt.z * added_pt.z < (80.0 * 80.0))
        {
          pl_surf.points.push_back(added_pt);
        }
      }
...
void Preprocess::rs_handler(const sensor_msgs::PointCloud2_<allocator<void>>::ConstPtr &msg)
{
  ...
  for (int i = 0; i < plsize; i++)
  {
    ...
    //移除机器狗自身的点云
    if(pl_orig.points[i].x < 0 && pl_orig.points[i].x > -1.0 && abs(pl_orig.points[i].y) < 0.5 && abs(pl_orig.points[i].z) < 1)
      continue;
  ...
  // cout<<"处理前点云数量:"<<pl_orig.points.size()<<" 处理后点云数量："<<pl_surf.points.size()<<endl;
  // for(int i=0;i<pl_surf.points.size()-1;i++)
  // {
  //   if(pl_surf.points[i].curvature>pl_surf.points[i+1].curvature)
